...
def temp_category(temps):
    ...
    for temp in temps:
        if temp >= 30:
            hot_count += 1
        elif temp >= 15:  # 15-29 range (covered by elif after >=30 check)
            mild_count += 1
        else:  # < 15
            cold_count += 1
...
def reverse_list(nums):
    # Build a new list by iterating from the last element to the first
    result = []
...
"""
def my_factorial(n):
    # Accumulator pattern: start at 1, multiply by each integer up to n
    result = 1
...
"""
def my_fib(n):
    # Start with the base Fibonacci sequence [0, 1]
